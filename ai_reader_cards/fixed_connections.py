...
class FixedLengthConnection:
    ...
    def __init__(self, parent_node, child_node, layout_type="mind_map"):
        ...
        # 固定长度参数
        self.horizontal_length = 80  # 水平线段长度
        self.vertical_length = 60    # 垂直线段长度
    ...
    def _create_l_shape_connection(self, parent_center, child_center, pw, ph, cw, ch):
        ...
        if abs(dx) > abs(dy):
            ...
            if dx > 0:
                ...
                self.path.lineTo(mid_x, start.y())  # 水平线段
                ...
                self.path = QPainterPath()
                self.path.moveTo(start)
                self.path.lineTo(mid_x, start.y())
                self.path.lineTo(mid_x, mid_y)
            ...
            if dy > 0:
                ...
                self.path = QPainterPath()
                self.path.moveTo(start)
                self.path.lineTo(start.x(), mid_y)  # 垂直线段
                ...
                self.path = QPainterPath()
                self.path.moveTo(start)
                self.path.lineTo(start.x(), mid_y)
                self.path.lineTo(mid_x, mid_y)
                self.path.lineTo(end)
