...
class MainController(QObject):
    ...
    def open_markdown_file(self, filepath):
        """打开Markdown文件"""
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
            filename = os.path.basename(filepath)
            self.status_updated.emit(f"已打开Markdown文件: {filename}")
            return content, filename
        except Exception as e:
            raise Exception(f"无法打开Markdown文件:\n{str(e)}")
    ...
    def _on_card_generated(self, card_data):
        ...
        # 保存源文本信息，用于跳转和高亮
        if "source_text" in card_data:
            card.source_text = card_data["source_text"]
