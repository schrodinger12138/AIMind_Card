...
class KnowledgeCard(QGraphicsRectItem):
    ...
    def add_sibling_card(self):
        ...
        if self.scene():
            ...
            # 如果有父节点，布局会在set_parent_card中自动应用
            # 如果没有父节点，需要手动应用布局
            if not self.parent_card:
                # 找到根节点并应用布局
                root_card = self
                while root_card.parent_card:
                    root_card = root_card.parent_card
                if hasattr(self.scene(), '_apply_layout_to_subtree'):
                    self.scene()._apply_layout_to_subtree(root_card)
            self.scene().update()
    ...
    def set_parent_card(self, parent):
        ...
        # 成为子节点后，根据布局自动更新连线和位置（参考madmap）
        if self.scene() and hasattr(self.scene(), '_apply_layout_to_subtree'):
            # 找到根节点
            root_card = self
            while root_card.parent_card:
                root_card = root_card.parent_card
