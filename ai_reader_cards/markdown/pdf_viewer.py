...
class PDFViewer(QWidget):
    ...
    def init_ui(self):
        ...
        self.zoom_combo.addItems(["50%", "75%", "100%", "125%", "150%", "200%", "250%", "300%", "400%"])
        ...
        # 翻页模式
        control_layout.addWidget(QLabel("翻页模式:"))
        self.page_mode_combo = QComboBox()
        self.page_mode_combo.addItems(["单页", "双页", "连续"])
        self.page_mode_combo.setCurrentText("单页")
        self.page_mode_combo.currentTextChanged.connect(self.on_page_mode_changed)
        control_layout.addWidget(self.page_mode_combo)
    ...
    def zoom_in(self):
        ...
        zoom_levels = [50, 75, 100, 125, 150, 200, 250, 300, 400]
        for zoom in zoom_levels:
            if zoom > current_zoom:
                self.zoom_level = zoom
                self.zoom_combo.setCurrentText(f"{zoom}%")
                if self.doc:
                    self.display_page(self.current_page)
                break
    ...
    def on_page_mode_changed(self, mode_text):
        """翻页模式改变"""
        # 这里可以添加不同的显示模式逻辑
        if self.doc:
            self.display_page(self.current_page)
